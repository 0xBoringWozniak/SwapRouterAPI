...
def low_size_router():
    orm = MockedORM()
    node = Web3(Web3.HTTPProvider(HTTP_NODE))
    return SwapRouter(trade=Trade(amount=10, token_in_symbol='USDT'), mongo_orm=orm, node=node)


def test_big_size_usdt(big_size_router):
    assert big_size_router.find_pool().address == '0xbEbc44782C7dB0a1A60Cb6fe97d0b483032FF1C7'
