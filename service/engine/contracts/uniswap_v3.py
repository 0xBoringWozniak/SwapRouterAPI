...
class UniswapV3Connector(Connector):
    ...
    def __init__(
        ...
        """
        Uniswap V3 connector. Uses Uniswap V3 Quoter contract.
        quoteExactInputSingle is used for getting amount out.
    ...
    def get_amount_out(self, amount_in: float) -> float:
        ...
        amount_out = self._contract.functions.quoteExactInputSingle(
            self._token0.address if self._zfo else
            self._token1.address,
            self._token1.address if self._zfo else
