...
class UniswapV2Connector(Connector):
    ...
    def get_amount_out(self, amount_in: float) -> float:
        ...
        amount_out = self._contract.functions.getAmountsOut(
            int(amount_in * (10**self._token0.decimals if self._zfo else 10**self._token1.decimals)),
            [self._token0.address, self._token1.address] if self._zfo else [self._token1.address, self._token0.address]
        ).call()[-1]
